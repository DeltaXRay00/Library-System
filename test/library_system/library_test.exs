defmodule LibrarySystem.LibraryTest do
  use LibrarySystem.DataCase

  alias LibrarySystem.Library

  describe "books" do
    alias LibrarySystem.Library.Book
  ...
  describe "loans" do
    ...
    test "create_loan/1 with valid data creates a loan" do
      valid_attrs = %{books: "some books", borrowed_at: ~N[2025-07-15 03:45:00], due_at: ~N[2025-07-15 03:45:00], returned_at: ~N[2025-07-15 03:45:00]}

      assert {:ok, %Loan{} = loan} = Library.create_loan(valid_attrs)
      assert loan.books == "some books"
      assert loan.borrowed_at == ~N[2025-07-15 03:45:00]
      assert loan.due_at == ~N[2025-07-15 03:45:00]
      assert loan.returned_at == ~N[2025-07-15 03:45:00]
    end
    ...
    test "update_loan/2 with valid data updates the loan" do
      loan = loan_fixture()
      update_attrs = %{books: "some updated books", borrowed_at: ~N[2025-07-16 03:45:00], due_at: ~N[2025-07-16 03:45:00], returned_at: ~N[2025-07-16 03:45:00]}

      assert {:ok, %Loan{} = loan} = Library.update_loan(loan, update_attrs)
      assert loan.books == "some updated books"
      assert loan.borrowed_at == ~N[2025-07-16 03:45:00]
      assert loan.due_at == ~N[2025-07-16 03:45:00]
      assert loan.returned_at == ~N[2025-07-16 03:45:00]
    end
