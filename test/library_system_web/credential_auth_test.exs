defmodule LibrarySystemWeb.CredentialAuthTest do
  use LibrarySystemWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias LibrarySystem.Credentials
  alias LibrarySystemWeb.CredentialAuth
  import LibrarySystem.CredentialsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, LibrarySystemWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_credential/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      LibrarySystemWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_credential/2" do
    ...
    test "authenticates credential from cookies", %{conn: conn, credential: credential} do
      ...
      assert get_session(conn, :live_socket_id) ==
               "profiles_sessions:#{Base.url_encode64(credential_token)}"
    end
  ...
  describe "on_mount :ensure_authenticated" do
    ...
    test "redirects to login page if there isn't a valid credential_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: LibrarySystemWeb.Endpoint,
