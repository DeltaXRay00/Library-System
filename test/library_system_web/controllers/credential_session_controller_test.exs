defmodule LibrarySystemWeb.CredentialSessionControllerTest do
  ...
  describe "POST /profiles/log_in" do
    ...
    test "logs the credential in with remember me", %{conn: conn, credential: credential} do
      ...
      assert conn.resp_cookies["_library_system_web_credential_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "DELETE /profiles/log_out" do
    test "logs the credential out", %{conn: conn, credential: credential} do
      conn = conn |> log_in_credential(credential) |> delete(~p"/profiles/log_out")
      assert redirected_to(conn) == ~p"/"
