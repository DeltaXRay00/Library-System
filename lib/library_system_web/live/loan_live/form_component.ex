defmodule LibrarySystemWeb.LoanLive.FormComponent do
alias LibrarySystem.Library.Loan
  use LibrarySystemWeb, :live_component
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:user_id]} type="select" label="Users" options={@users} />
        <.input field={@form[:book_id]} type="select" label="Book" options={@books} />
  ...
  def update(%{loan: loan} = assigns, socket) do
    ...
    changeset = Library.change_loan(loan)
