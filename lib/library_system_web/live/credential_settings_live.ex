defmodule LibrarySystemWeb.CredentialSettingsLive do
    ...
    <div class="space-y-12 divide-y">
      ...
      <div>
        <.simple_form
          for={@password_form}
          id="password_form"
          action={~p"/profiles/log_in?_action=password_updated"}
          method="post"
  ...
  def mount(%{"token" => token}, _session, socket) do
    ...
    {:ok, push_navigate(socket, to: ~p"/profiles/settings")}
  end
    ...
    case Credentials.apply_credential_email(credential, password, credential_params) do
      {:ok, applied_credential} ->
        Credentials.deliver_credential_update_email_instructions(
          applied_credential,
          credential.email,
          &url(~p"/profiles/settings/confirm_email/#{&1}")
        )
