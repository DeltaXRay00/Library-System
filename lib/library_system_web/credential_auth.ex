defmodule LibrarySystemWeb.CredentialAuth do
  use LibrarySystemWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias LibrarySystem.Credentials

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in CredentialToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_library_system_web_credential_remember_me"
  ...
  def log_out_credential(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LibrarySystemWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_credential:
      ...
      defmodule LibrarySystemWeb.PageLive do
        use LibrarySystemWeb, :live_view

        on_mount {LibrarySystemWeb.CredentialAuth, :mount_current_credential}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{LibrarySystemWeb.CredentialAuth, :ensure_authenticated}] do
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    ...
    if socket.assigns.current_credential do
      ...
      socket =
        ...
        |> Phoenix.LiveView.redirect(to: ~p"/profiles/log_in")
